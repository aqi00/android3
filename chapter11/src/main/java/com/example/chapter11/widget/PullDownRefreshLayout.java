...
import com.example.chapter11.widget.PullDownScrollView.ScrollListener;
...
public class PullDownRefreshLayout extends LinearLayout implements View.OnTouchListener, ScrollListener {
    ...
    private PointF mOriginPos; // 落点的位置
    ...
    public PullDownRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mContext = context;
        // 触发工具栏变色的临界滑动距离
        mCriticalDistance = Utils.dip2px(mContext, 20);
        // 获取默认的下拉刷新头部布局
        mLinearLayout = (LinearLayout) LayoutInflater.from(mContext).inflate(R.layout.header_drag, null);
        // 计算下拉刷新头部布局的高度
        ...
        mLinearLayout.postInvalidate(); // 立刻刷新线性布局（线程安全方式）
        addView(mLinearLayout, 0); // 把下拉刷新头部布局添加到最前面
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) { // 按下手指
            mOriginPos = new PointF(event.getRawX(), event.getRawY());
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        if (event.getRawY() <= mOriginPos.y) { // 往上拉动，不处理
            return false;
        } else if (mScrollView.getScrollY() > 0) { // 未拉到顶部，不处理
            return false;
        } else if (mScrollView.getScrollY() <= 0) { // 正在下拉，则隐藏工具栏
            mListener.hideTitle(); // 隐藏标题栏
        }
        // 计算下拉过程的拉动距离
        float offsetY = event.getRawY() - mOriginPos.y - mCurrentHeight;
        Log.d(TAG, "mCurrentHeight=" + mCurrentHeight + ",offsetY=" + offsetY + ",getScrollY=" + mScrollView.getScrollY());
        if (action == MotionEvent.ACTION_DOWN) { // 按下手指
            mOriginPos.y = event.getRawY();
