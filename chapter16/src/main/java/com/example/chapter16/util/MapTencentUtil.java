package com.example.chapter16.util;
...
public class MapTencentUtil {
    ...
    public static LatLng getCenterPos(List<LatLng> posList) {
        double totalLatitude = 0;
        double totalLongitude = 0;
        int count = posList.size();
        for (int i = 0; i < count; i++) {
            totalLatitude += posList.get(i).latitude;
            totalLongitude += posList.get(i).longitude;
        }
        LatLng centerPos = new LatLng(totalLatitude / count, totalLongitude / count);
        return centerPos;
    }

    public static List<MapPoint> calculatePoint(List<LatLng> posList) {
        LatLng centerPos = getCenterPos(posList);
        List<MapPoint> pointList = new ArrayList<>();
        for (int i = 0; i < posList.size(); i++) {
            LatLng pos = posList.get(i);
    ...
    public static double getAreaByLatLng(List<LatLng> posList) {
        List<MapPoint> pointList = calculatePoint(posList);
        return getArea(pointList);
    }
    ...
    public static boolean isInsidePolygon(LatLng pos, List<LatLng> posList) {
        mCross = new Cross();
        for (int i = 0; i < posList.size(); i++) {
