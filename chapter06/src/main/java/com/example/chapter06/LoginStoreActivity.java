...
public class LoginStoreActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 获取数据仓库保存的手机号码
        String phone = datastore.getStringValue("phone");
        // 获取数据仓库保存的密码
        String password = datastore.getStringValue("password");
    ...
    private class HideTextWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable s) {
            ...
            if ((str.length() == 11 && mMaxLength == 11)
                ...
                ViewUtil.hideOneInputMethod(LoginStoreActivity.this, mView); // 隐藏输入法软键盘
            }
        }
    }
    ...
    private void loginSuccess() {
        ...
        if (isRemember) {
            DatastoreUtil datastore = DatastoreUtil.getInstance(this); // 获取数据仓库工具的实例
            datastore.setStringValue("phone", et_phone.getText().toString()); // 添加名叫phone的手机号码
            datastore.setStringValue("password", et_password.getText().toString()); // 添加名叫password的密码
        }
    }
