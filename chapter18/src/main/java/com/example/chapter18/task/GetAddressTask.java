...
public class GetAddressTask extends Thread {
    ...
    public void run() {
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, final Response response) throws IOException { // 请求成功
                ...
                Log.d(TAG, "resp="+resp);
                // 下面从json串中逐级解析获得省份、地市、区县、地址
                try {
                    JSONObject obj = new JSONObject(resp);
                    JSONObject result = obj.getJSONObject("result");
                    JSONObject detail = result.getJSONObject("addressComponent");
                    String province = detail.getString("province"); // 省份
                    String city = detail.getString("city"); // 地市
                    String county = detail.getString("county"); // 区县
                    String address = detail.getString("address"); // 地址
                    CityInfo cityInfo = new CityInfo(province, city, county, address);
                    // 回到主线程操纵界面
                    mAct.runOnUiThread(() -> mListener.onFindAddress(cityInfo));
    ...
    public interface OnAddressListener {
        void onFindAddress(CityInfo cityInfo);
    }
